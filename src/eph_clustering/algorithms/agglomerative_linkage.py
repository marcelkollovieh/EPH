...
def agglomerative_linkage(
    ...
    if check:
        graph_copy = nx.convert_node_labels_to_integers(
            graph_copy
        )  # converts dps labels to integers starting from 0

        if affinity == "unitary":
            for e in graph_copy.edges():
                graph_copy.add_edge(
                    e[0], e[1], weight=1
                )  # still same number of edges if they were already there
        ...
        if affinity == "weighted" and not n_weighted_edges == n_edges:
            raise KeyError(
                "%s edges among %s do not have the attribute/key 'weight'."
    ...
    while n_nodes > 0:
        ...
        while chain:
            ...
            if chain:
                ...
                if b == c:
                    dendrogram.append(
                        [a, b, f(linkage), cluster_size[a] + cluster_size[b]]
                    )
                    graph.add_node(u)
