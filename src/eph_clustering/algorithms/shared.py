...
from eph_clustering.util import chunker

PreprocessedGraph = namedtuple(
    "PreprocessedGraph",
    field_names=[
        "node_ids",
        "p_uv",
...
def preprocess_graph(adjacency, deg=None, subset=None, normalize=True):
    ...
    if deg is None:
        deg_out = (
            adjacency @ torch.ones([num_nodes, 1], device=device, dtype=dtype)
        ).squeeze(1)
        deg_in = (
            adjacency.t() @ torch.ones([num_nodes, 1], device=device, dtype=dtype)
        ).squeeze(1)
    else:
        assert len(deg) == 2
        deg_out, deg_in = deg
    ...
    if subset is not None:
        edge_ixs, edge_weight = subgraph(
            subset, edge_ixs.t(), edge_weight, num_nodes=num_nodes, relabel_nodes=True
        )
        ...
        adjacency = torch.sparse_coo_tensor(
            (edge_ixs.t()),
            values=torch.ones(edge_ixs.shape[0], device=device, dtype=dtype),
            size=[subset.shape[0], subset.shape[0]],
        ).coalesce()
        deg_out = (
            adjacency @ torch.ones([subset.shape[0], 1], device=device, dtype=dtype)
        ).squeeze(1)
        deg_in = (
            adjacency.t() @ torch.ones([subset.shape[0], 1], device=device, dtype=dtype)
        ).squeeze(1)
    else:
    ...
    if normalize:
        ...
        p_uv_drop = edge_weight.to(dtype)
        p_u_out_drop = deg_out / deg_out.sum()
        p_u_in_drop = deg_in / deg_in.sum()
    return PreprocessedGraph(
        node_ids=subset,
        p_uv=p_uv_drop.to(device),
        p_u_out=p_u_out_drop.to(device),
        p_u_in=p_u_in_drop.to(device),
        edge_ixs=edge_ixs.to(device),
        adj=adjacency.to(device),
        mutual_information=p_uv_drop
        @ torch.log(
            p_uv_drop / (p_u_in_drop[edge_ixs[:, 0]] * p_u_out_drop[edge_ixs[:, 1]])
        ),
    )
...
def compute_dasgupta(A, B, adj=None, preprocessed_adj=None):
    ...
    adj: torch.sparse_coo_tensor, shape [n_nodes, n_nodes], optional
    ...
    m_sum = A.to(device) @ (
        B_anc + torch.eye(B_anc.shape[0], device=device, dtype=dtype)
    )
    ...
    if len(edge_ixs) > 1e6:
        ...
        fun = (
            lambda m_sum, edge_ixs, B_square_inv, edge_weights: (
                m_sum[edge_ixs[:, 0]].mul(m_sum[edge_ixs[:, 1]]) @ B_square_inv
            )
            .mul(m_sum.sum(0)[None, :])
            .sum(-1)
            .mul(edge_weights.to(device))
            .sum(0, keepdims=True)
        )
        dasgupta = torch.cat(
            [
                checkpoint(
                    fun, m_sum, edge_ixs[chunk], B_square_inv, edge_weights[chunk]
                )
        ...
        dasgupta = (
            lca_probs.mul(soft_cardinalities[None, :])
            .sum(-1)
            .mul(edge_weights.to(device))
            .sum()
        )
...
def compute_p(
    ...
    if len(edge_ixs) > 1e5:
        ...
        fun = (
            lambda m_sum, edge_ixs, B_square_inv, p_uv: (
                m_sum[edge_ixs[:, 0]].mul(m_sum[edge_ixs[:, 1]]) @ B_square_inv
            )
            .mul(p_uv[:, None])
            .sum(0, keepdims=True)
        ...
        p_x = torch.cat(
            [
                checkpoint(fun, m_sum, edge_ixs[chunk], B_square_inv, p_uv[chunk])
                for chunk in edge_chunks
            ]
        ).sum(0)
    else:
...
def compute_q(A, B, P_u, B_anc=None, B_square_inv=None, same_leaf_correction=False):
    """
...
def compute_TSD(
    ...
    p_x = compute_p(
        A, B, edges_sampled, p_uv_drop, B_anc=B_anc, B_square_inv=B_square_inv
    )
    q_x = (
        compute_q(
            A,
            B,
            (p_u_out_drop, p_u_in_drop),
            B_anc=B_anc,
            B_square_inv=B_square_inv,
            same_leaf_correction=same_leaf_correction,
        )
        + 1e-7
    )
    if q_x.min() < 0:
